...
Structure can be enforced **during** generation (constrained decoding) or **after** generation (post-hoc validation). Outlines builds an index over a model's vocabulary and masks any token that would violate a supplied regex, JSON Schema, or Pydantic model, so that constraining token selection during generation "guarantees structured outputs during generation — directly from any LLM," rather than "attempt[ing] to fix bad outputs after generation using parsing, regex, or fragile code that breaks easily" [3]. OpenAI's Structured Outputs takes the same decode-time approach for its API. Its published evaluation reports 100% schema adherence for `gpt-4o-2024-08-06` and less than 40% for `gpt-4-0613` in that evaluation [5]. That is a scoped schema-conformance result, not a like-for-like comparison with ordinary function calling or a claim about field-level correctness.
...
Once a document already exists on disk and is no longer being generated turn-by-turn, decode-time constraints cannot be applied retroactively. Post-hoc validation still can: Guardrails' own README demonstrates `guard.validate(existing_text)` against an already supplied string [4]. Checking a 160-entry file is therefore a post-hoc task, and deterministic prose/doc linters are a better fit when the contract is structural. Vale reads Markdown, AsciiDoc, reStructuredText, and other formats, applying rules through YAML-defined extension points — sequence matching, conditional rules, consistency checks — entirely offline, no LLM in the loop [6]. Markdownlint similarly ships roughly 60 numbered rules (MD001–MD060, some retired) as a deterministic structural checker built on the CommonMark-compliant micromark parser [8].
...
Counting, not just flagging, turns the lint into a trend instrument: a class going from 31 to 34 signals the writer's instructions are still leaking; 31 to 0 confirms the normalize-in-place pass worked. Mutation testing leans on the same instinct with its "percentage of mutants killed" metric [11][12] — a single pass/fail bit throws away the trend information.

The hint-vs-violation split matters because a lint has to survive contact with legitimate exceptions. If "Domain must be single-valued" hard-fails every `A/B` entry, the team has two options: shrink detection (stop looking, losing real information) or accept some `A/B` entries as legitimate and downgrade the verdict. The generalizable principle: detection stays maximally sensitive, and the judgment call about whether a detected pattern is acceptable belongs in the verdict layer, not the detection layer. ESLint offers a useful separation: `--fix-type` selects categories of changes only when a caller also requests `--fix` or `--fix-dry-run`; it does not change which problems the rules detect [9]. Whether a particular rule has a safe fix remains rule-specific.
...
- **RFC numbers** are permanent once assigned; an obsoleted RFC is superseded by a *new* number, and old numbers are never reassigned — which is why "RFC 2119" means the same thing in any document, any year [14].
- **Wikidata QIDs** are documented as persistent identifiers that never change once assigned and survive name changes or merges [15] — though whether a *deleted* item's QID is ever reissued to an unrelated entity could not be confirmed here and should be treated as unverified.
- **Zettelkasten IDs**, following Niklas Luhmann's tradition, exist for the same reason: "only with a unique identifier you can address Zettel individually," and time-based IDs are preferred over title-based ones because they let you "change the title as much as you want without breaking any links" — the identifier's job is to be independent of the note's current position or label [13].
- **Architecture Decision Records**, per the convention Michael Nygard popularized, are numbered sequentially and superseded rather than renumbered: an old ADR is marked "Superseded by ADR-NNN," the new one records "Supersedes ADR-MMM," and historical numbering never shifts [10].

The common thread: an identifier's stability guarantee must be stronger than the guarantee any one reorganizing edit will respect. A section-grouping pass that "just" reorganizes for readability is, to a reference, a silent renumbering event — exactly what happened here. The fix — realigning cross-references using each reference's parenthesized title as an oracle — mirrors reference-integrity tooling elsewhere: Sphinx's `:ref:` system is preferred over plain links because it tracks labels rather than raw positions and "will raise warnings if incorrect" when a target goes missing or is renamed [7]. The general principle for any derived document: it must share the source's identifiers, or carry an explicit, versioned map between the two ID spaces — chosen deliberately, not left to accident.

## Report-Only vs Auto-Fix

Why report-only, when ESLint and Prettier can normalize entire codebases when a caller explicitly opts into autofix? It turns on what kind of "correct" is being enforced. ESLint's `--fix` instructs it to try to fix as many issues as possible, but not every problem is fixable and the resulting changes are written to the actual files [9]. Rewriting `*` to `-` for list bullets is lossless and unambiguous.
...
5. Introducing Structured Outputs in the API — OpenAI (published schema-adherence evaluation). https://openai.com/index/introducing-structured-outputs-in-the-api/
6. Vale — "Your style, our editor" (prose/docs linter, rule-based, offline). https://vale.sh/
7. Cross-references — Sphinx documentation (`:ref:` labels, dangling-reference warnings, `:any:` role). https://www.sphinx-doc.org/en/master/usage/referencing.html
8. markdownlint — GitHub repository, DavidAnson/markdownlint (rule count, CommonMark/micromark basis, autofix support). https://github.com/DavidAnson/markdownlint
9. ESLint Command Line Interface Reference (`--fix`, `--fix-type`, and their opt-in/rule-specific limits). https://eslint.org/docs/latest/use/command-line-interface
10. Architectural Decision Records (ADR) — adr.github.io (decision log; numbering convention verified via secondary sources referencing Michael Nygard's 2011 post). https://adr.github.io/
11. What is mutation testing? — Stryker Mutator documentation. https://stryker-mutator.io/docs/
12. Stryker Mutator — home page (coverage vs. mutation testing, "chocolate paste" analogy). https://stryker-mutator.io/
13. Introduction to the Zettelkasten Method — zettelkasten.de (unique identifiers, time-based vs. title-based IDs). https://zettelkasten.de/introduction/
14. RFC Editor — "What Is an RFC?" (RFC numbers are permanent and never reused). https://www.rfc-editor.org/series/rfc/
15. Wikidata:Identifiers — Wikidata (QIDs as persistent, unchanging identifiers). https://www.wikidata.org/wiki/Wikidata:Identifiers
**Unverified claims flagged in text:** whether a deleted Wikidata QID is ever reissued to a different, unrelated entity was not confirmed by primary documentation, so the Stable Identifiers section explicitly leaves it unverified.
