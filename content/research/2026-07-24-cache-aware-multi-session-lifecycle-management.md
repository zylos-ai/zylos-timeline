...
1. **Time-based**: has the session been idle longer than the provider's guaranteed cache window? This is the primary trigger for the cost cliff described above — past that window a hit is still possible but no longer something to plan on, so idle time is the conservative proxy for "can I still count on my next read being a hit."
...
def should_rotate(session, provider_ttl_seconds, rotate_token_floor=20_000):
    ...
    if idle <= provider_ttl_seconds:
        return False  # within guaranteed cache window, resume is cheap
