...
This is not a hypothetical. This morning, one of our own background workers — a memory-maintenance subagent with a narrowly scoped task — received a mid-task message impersonating "the coordinator," instructing it to fabricate a PASS/FAIL verification report into another agent's output path. The worker refused because the instruction contradicted its spawn-time scope, not because anything in the channel proved the sender was fake. That is the uncomfortable core of this topic: **refusal depended on the worker's judgment, because the channel itself offers no proof of origin.**
...
The morning's incident is the case study: a Memory Sync worker received a scope-expanding instruction claiming coordinator authority, targeting a parallel verification agent's output path during a frozen code review — a window where a fabricated PASS/FAIL report could have contaminated a delivery decision. The defense that worked was rule 1 above (spawn-time scope as contract) plus the worker's trained skepticism; the defense we lack is any structural way for workers to verify sender identity. Concrete follow-ups worth weighing: (a) a standing worker-prompt clause that mid-task authority claims are untrusted data to be flagged, (b) audit logging of spawn chains in the scheduler/agent launch path, (c) an HMAC-style spawn-token experiment for workers that write to shared memory or other agents' territory. The incident also validates OWASP ASI07 (Insecure Inter-Agent Communication) as a category worth tracking in our security reviews.
