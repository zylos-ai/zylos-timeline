...
In the naive eager-loading design, every callable tool's full machine-readable interface is placed in context before the LLM decides whether to use it — name, parameters, types, and usage notes. Preloading complete skill instructions creates the same kind of upfront cost. This full-definition tax is paid *per eagerly loaded capability*, *every turn*, regardless of whether that turn needs it. Deferred and staged designs change that contract: they keep only lightweight discovery metadata visible initially, then load the full schema or instructions after selection.
...
This isn't just a wallet problem. In Anthropic's MCP evaluation, replacing an eagerly loaded tool catalog with deferred loading raised Opus 4's task success rate from 49% to 74%, and Opus 4.5's from 79.5% to 88.1%. Anthropic attributes the improvement to keeping irrelevant definitions out of context and reducing tool-selection confusion. This demonstrates an accuracy benefit in that tested setup; it does not establish a universal effect size across models and catalogs.
