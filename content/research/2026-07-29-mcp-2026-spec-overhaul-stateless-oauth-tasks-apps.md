...
- **Header-based routing** — [Streamable HTTP routing headers](https://modelcontextprotocol.io/specification/2026-07-28/basic/transports/streamable-http#standard-request-headers) require `Mcp-Method` on all requests; `Mcp-Name` is required only for `tools/call`, `resources/read`, and `prompts/get`. Gateways can route and meter traffic without parsing JSON bodies, but must not require `Mcp-Name` on list, discovery, or task-status requests.
...
- **Header-based secret leakage** — the routing headers (`Mcp-Method` on all requests; `Mcp-Name` only on `tools/call`, `resources/read`, and `prompts/get`) create a temptation to stuff sensitive values (API keys, tokens, PII) into headers, which then get logged by infrastructure (proxies, WAFs, load balancers) that isn't designed with MCP-specific redaction in mind.
