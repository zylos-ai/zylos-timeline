...
Where the underlying content isn't a live-editable CRDT/OT document — e.g., annotating arbitrary web pages you don't control — the **W3C Web Annotation Data Model** formalizes "anchoring" via typed Selectors (`TextQuoteSelector`, `TextPositionSelector`, `FragmentSelector`, `RangeSelector`) that describe *how* to relocate a target within a document ([Web Annotation Data Model, W3C Recommendation](https://www.w3.org/TR/annotation-model/)). **Hypothesis** implements the pragmatic, degraded-content case: it stores three selectors per anchor (range, text-position, text-quote-with-32-chars-context) and falls back through them in order, using a Bitap/Myers-diff-based fuzzy text search when the page has changed enough that exact positions no longer resolve ([Fuzzy Anchoring, Hypothesis](https://web.hypothes.is/blog/fuzzy-anchoring/)). That source establishes fallback matching, not a guarantee that the UI visibly labels approximate relocation. Pattern 2 below recommends such labeling for a review product; it is not an attributed Hypothesis UI behavior.

### 3. HTTP-Layer Machinery: ETag/If-Match and Idempotency Keys

HTTP conditional requests can guard a write against changes to its target representation. RFC 9110 §13.1.1 defines `If-Match` against the selected representation of the request target, using strong comparison; a failed precondition normally produces `412 Precondition Failed` ([RFC 9110](https://www.rfc-editor.org/rfc/rfc9110.html#section-13.1.1)). An ETag obtained from `GET /documents/A` does not automatically guard A's revision when sent to `POST /documents/A/comments`: those are different targets. The endpoint must define a representation/validator contract that actually covers the reviewed source, or accept an explicit expected document revision and enforce it with the comment insert in a transaction. Merely forwarding the header does not establish that contract.
...
The Model Context Protocol's July 2026 release candidate makes the transport **stateless**, removing the `Mcp-Session-Id` header and protocol-level sessions entirely, with every request instead carrying explicit version/capability metadata in `_meta` ([2026-07-28 MCP Specification Release Candidate](https://blog.modelcontextprotocol.io/posts/2026-07-28-release-candidate/); [Key Changes changelog](https://modelcontextprotocol.io/specification/2026-07-28/changelog)). This is directionally aligned with view-snapshot binding: rather than relying on implicit, stateful "current session" context (which is exactly the anti-pattern that causes S1/S2 drift), the protocol is pushing toward requests that self-describe their exact version context. Protocol-version metadata does not identify the version of an individual resource. The [2026-07-28 resource-content schema](https://modelcontextprotocol.io/specification/2026-07-28/schema) carries a URI, optional MIME type and metadata, plus text or blob content, but defines no standardized source-revision precondition for a later approval action. An application may extend that metadata or use immutable resource URIs, but must define and enforce the corresponding approval contract.

Preloop's control-plane article describes approval context capture, including tool arguments, agent justification, and the matched policy ([Preloop](https://preloop.ai/resources/ai-agent-control-plane-2026)). That supports an audit-context recommendation, not a claim of immutable policy-version binding. For the proposed workflow here, retain the applicable policy version and the reviewed subject/version explicitly.

LangGraph supplies durable pause/resume through checkpoints and `interrupt()`. Its docs describe `thread_id` as a persistent cursor, with resumption via application-supplied `Command(resume=...)` input ([Interrupts](https://docs.langchain.com/oss/python/langgraph/interrupts); [Persistence](https://docs.langchain.com/oss/python/langgraph/persistence)). A checkpoint preserves graph state; a filename stored in that state can still resolve to different external bytes later. The application therefore needs to checkpoint the reviewed subject identifier and immutable version/digest, validate the approver's authority when accepting a decision, and enforce the bound subject at execution. Pause/resume and snapshot-bound authorization are distinct responsibilities.
...
**4. Idempotency-Key + Atomic Version Gate.** Require a client-generated key, the served snapshot ID, and an explicit expected source revision. Replay a matching authorized completed request first; for a new request, compare the revision and create the comment within a transaction that excludes concurrent document edits. An HTTP endpoint may expose a strong `If-Match` validator for its defined target representation; a document validator cannot simply be reused for a separate comments target without the corresponding contract. An opaque served-view ID is not automatically a validator. The explicit source-revision transaction below avoids relying on that assumption. This pattern chooses a strict current-at-creation policy. Historical comments are another valid design, but must be explicitly bound to the old snapshot and labeled outdated, not described as current approvals. *Trade-off:* new stale submissions get a conflict and require re-review; retries of an already accepted submission return their original result.
...
If an agent or human edits the source while a reviewer is reading, submission-time resolution of “latest” can bind the decision to different content. Automated regeneration is one possible source of such mutation; the cited agent-security survey does not establish a comparative incident rate against human-only review. The relevant condition is mutation during the review-to-execution interval, regardless of who performs it. Three consequences follow:
...
- **MCP resource-content preconditions need an application contract in the version surveyed here.** With the 2026-07-28 MCP spec moving toward stateless, self-describing requests, platform builders integrating document/resource review over MCP should not assume the protocol gives them staleness detection — they need to add their own content-hash or revision-ID field to resource payloads and treat it as load-bearing, the same way they'd treat an ETag.
...
- **Interoperable approval contract in the surveyed stack.** HTTP validators can cover application-defined source representations, and [W3C Annotation State](https://www.w3.org/TR/annotation-model/#states) describes how to obtain the appropriate representation, including temporal state and cached copies. The [MCP 2026-07-28 resource-content schema](https://modelcontextprotocol.io/specification/2026-07-28/schema) has no standardized source-revision precondition for a later approval action. How should an application profile connect the served subject/version, authorized decision, and execution-time check across these primitives? This is a concrete integration question, not a claim that no relevant standard exists.
...
- [Schema Reference — MCP 2026-07-28](https://modelcontextprotocol.io/specification/2026-07-28/schema)
