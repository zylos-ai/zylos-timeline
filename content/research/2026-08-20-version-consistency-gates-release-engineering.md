...
A release has one version number but usually many places that claim to state it: the package manifest, the lockfile, one or more metadata/registry descriptors, the changelog header, sometimes a `__version__` string or a Helm chart. When these "version faces" disagree, the failure can stay silent until a downstream consumer reads the wrong face. The motivating case is an anonymized internal incident observed on a component-platform team: a release bumped 2 of 4 required version files; an installed-version registry read a stale descriptor and generated repeat-upgrade prompts for users who had already upgraded. This is operational experience from that team, not an independently published incident report. The analysis found two layers: release consistency existed only as documentation policy, and the project's scaffold had copied only a subdirectory of the template, leaving later template rules and tests without an automatic propagation path into the generated repo.

This article surveys how mature release tooling (release-please, changesets, semantic-release, npm's `version` lifecycle, cargo-release, bump2version/bumpver) keeps multi-file versions synchronized; how lockfile drift specifically breaks CI; how to build a permanent consistency-gate test with positive controls (known-bad mutants) so the gate can be proven to fail; where the boundary sits between what a machine gate can prove and what only human/process review can prove; and how scaffold tools differ in whether they let engineering rules propagate into already-generated projects over time, or freeze them at generation time. It closes with concrete recommendations for agent-component platforms, which are exactly the kind of environment — many small generated repos, each carrying release/version machinery copied from a shared template — where this failure mode recurs.
...
**cargo-release** (Rust) automates the crates.io flow: bump `Cargo.toml`, test, tag, publish, and optionally move to a next development version. Its README notes a narrower caveat under **dry run**: because the dry-run bump is held in memory, packaging/verification output can still reflect the pre-bump version. That statement does not generalize to `--execute` or to arbitrary hooks. `Cargo.lock` also does not affect downstream consumers of a *library* the way `Cargo.toml` does (Cargo.toml vs Cargo.lock, The Cargo Book) — not every lockfile is an externally meaningful version face.

**bump2version / bumpversion / bumpver** (Python ecosystem, but language-agnostic in practice) use a "single source of truth, many write targets" configuration: a `.bumpversion.cfg` can list multiple file sections and one command performs their replacements. In bump2version, the default replacement uses an unrestricted string replacement and can update repeated identical version strings in one file; custom `search`/`replace` is useful when only selected occurrences should change. The multi-file operation is not atomic: after preflight, the implementation iterates targets and writes them individually, so interruption or a later write failure can leave a partially changed tree. Commit and tag creation are optional follow-on steps, not rollback protection for those writes.

Across the tools surveyed, the consistent lesson is to pick one source of truth and make every other registered face a generated artifact of a release step rather than a second value edited by hand.
...
# Maintained from the release contract, independently of whichever keys an
# extractor happened to return in this run.
REQUIRED_VERSION_FACES = frozenset({
    "manifest", "lockfile", "registry_descriptor", "changelog"
})

def check_consistent(versions: dict[str, str | None],
                     required_faces=REQUIRED_VERSION_FACES) -> Result:
    absent = sorted(required_faces - versions.keys())
    unreadable = sorted(k for k in required_faces
                        if k in versions and versions[k] is None)
    missing = absent + unreadable
    if missing:
        return Result.fail(f"version face(s) missing or unreadable: {missing}")
    present = {k: versions[k] for k in required_faces}
...
def test_positive_control_face_missing_entirely():
    # Real omission: the required registry_descriptor key is absent, not None.
    result = check_consistent({"manifest": "1.4.0", "lockfile": "1.4.0",
                                "changelog": "1.4.0"})
    assert not result.ok
    assert "registry_descriptor" in result.message
...
The positive-control known-bad fixtures should be checked-in files (or literal dicts, as above) that are never "fixed". The required-face inventory must be maintained independently from the extractor's runtime result; otherwise deleting a configured key can shrink the observed set and make omission pass. When a fifth required face is added, update the release-contract inventory and extraction configuration, then retain positive controls for both an absent key and a stale value, plus a negative-control known-good baseline. This gate proves agreement and presence only for the inventoried faces. It cannot discover an arbitrary new consumer or version-bearing file that nobody registers; repository ownership and release review still have to keep that inventory complete.

## Machine Gate vs. Process Gate: What a Working-Tree Check Can and Cannot Prove

The consistency test above proves a **working-tree invariant** limited to its independently maintained required-face inventory: at the commit under test, those registered faces are present, readable, and agree. It cannot discover unknown, unregistered consumers, and it cannot answer several related process questions:
...
- **Cookiecutter / degit / GitHub template repositories** are primarily one-shot generation mechanisms; ongoing synchronization needs another layer such as Cruft or a project-specific update process. **Yeoman is more programmable than that grouping suggests:** its generators can locate an existing project through `.yo-rc.json`, resolve conflicts against existing files, and implement AST-based updates or migrations. What Yeoman does not automatically provide is Copier-style template-history reconciliation; rerunnable behavior depends on the generator author implementing it.
- **Copier** treats scaffolding as an ongoing lifecycle. It records template/answer state and `copier update` compares the prior template state, a selected newer template revision, and the current project, surfacing conflicts for review. Git-backed templates are required for updates, but the selected revision need not be a tag: `--vcs-ref=HEAD` or another Git ref is allowed, while tags are part of the default revision-selection path. This supports reapplying template changes; it does not unconditionally guarantee that every local edit is preserved without conflict or manual resolution.
...
1. **Make version consistency a machine gate, not documentation, in every generated repo.** Ship a pure consistency-check test with an independently maintained required-face inventory, permanent positive controls for both stale values and truly absent keys, and a negative-control known-good baseline. Wire it into CI as a required check, and prefer a release tool that mechanically bumps every registered face. The guarantee remains scoped to that inventory; adding a new consumer requires an explicit inventory-and-control update.
...
4. **Treat the positive-control known-bad fixtures as permanent, protected artifacts.** Require the same review scrutiny to modify or delete a known-bad fixture as to modify the checker's pass-path logic — deleting an inconvenient positive control is functionally identical to disabling the gate, just less visible in a diff.
...
- [npm docs — npm version](https://docs.npmjs.com/cli/v11/commands/npm-version)
- [npm/libnpmversion — library implementing `npm version` behavior](https://github.com/npm/libnpmversion)
- [cargo-release — README and dry-run behavior](https://github.com/crate-ci/cargo-release#dry-run)
- [The Cargo Book — Cargo.toml vs Cargo.lock](https://doc.rust-lang.org/cargo/guide/cargo-toml-vs-cargo-lock.html)
- [bump2version — replacement implementation](https://github.com/c4urself/bump2version/blob/master/bumpversion/utils.py)
- [bump2version — CLI multi-file flow](https://github.com/c4urself/bump2version/blob/master/bumpversion/cli.py)
- [Yeoman authoring: file system, conflicts, and project-root behavior](https://yeoman.io/authoring/file-system.html)
- [Copier updating guide](https://github.com/copier-org/copier/blob/master/docs/updating.md)
