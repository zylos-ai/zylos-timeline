...
The negative-control fixtures should be checked-in files (or literal dicts, as above) that are never "fixed". The required-face inventory must be maintained independently from the extractor's runtime result; otherwise deleting a configured key can shrink the observed set and make omission pass. When a fifth required face is added, update the release-contract inventory and extraction configuration, then retain controls for both an absent key and a stale value. This gate proves agreement and presence only for the inventoried faces. It cannot discover an arbitrary new consumer or version-bearing file that nobody registers; repository ownership and release review still have to keep that inventory complete.
...
1. **Make version consistency a machine gate, not documentation, in every generated repo.** Ship a pure consistency-check test with an independently maintained required-face inventory and permanent negative controls for both stale values and truly absent keys. Wire it into CI as a required check, and prefer a release tool that mechanically bumps every registered face. The guarantee remains scoped to that inventory; adding a new consumer requires an explicit inventory-and-control update.
